...
function overlapDays(start, end, fromStr, toStr) {
  const s = new Date(Math.max(new Date(start), new Date(fromStr)));
  const e = new Date(Math.min(end ? new Date(end) : new Date(), new Date(toStr)));
  if (e <= s) return 0;
  return Math.max(1, Math.ceil((e - s) / (1000 * 60 * 60 * 24)));
}

export default function CampaignConnectSidebar({
  campaign,       // { id, daily_rate_usd, start_date, end_date }
  platformLabel,
  existingConn,   // null | { id, item_type, item_id, delivery_cost }
  products,
  packs,
  offers,
  usdRate,
  ...
  // Campaign spend over selected date range
  const campaignSpend = campaign
    ? overlapDays(campaign.start_date, campaign.end_date, adsFrom, adsTo) * campaign.daily_rate_usd * usdRate
    : 0;
  ...
  const prices = (() => {
    ...
    if (itemType === 'pack') {
      ...
      if (!pk) return null;
      // buy price: first preset items total OR product variant × item_count
  ...
  // Load stats when item or date range changes
  useEffect(() => {
    if (!itemId) { setStats(null); return; }
    setStatsLoading(true);
    getCampaignItemStats(itemType, parseInt(itemId), adsFrom, adsTo)
      .then(r => setStats(r.data))
      .catch(() => setStats(null))
      .finally(() => setStatsLoading(false));
  }, [itemType, itemId, adsFrom, adsTo]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await onSave({ campaign_id: campaign.id, item_type: itemType, item_id: parseInt(itemId), delivery_cost: delivery });
      onClose();
    } finally {
      setSaving(false);
    }
    ...
    <div style={sidebarStyle}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: '#8892b0' }}>
              {platformLabel} · {campaign?.start_date?.slice(0, 10)}
              {campaign?.end_date ? ` → ${campaign.end_date.slice(0, 10)}` : ' (running)'}
            </div>
            <div style={{ fontSize: 12, color: '#a78bfa', marginTop: 3 }}>
              Spend in range: {fmt(campaignSpend)} MAD
            </div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#8892b0', cursor: 'pointer', padding: 4 }}>
            <X size={18} />
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        {prices && (
          <div>
            ...
            <div style={{ background: '#13151e', borderRadius: 10, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 0 }}>
              <div style={rowStyle}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      border: '1px solid #2d3248', background: '#1d1d27',
              ...
              <div style={rowStyle}>
                <span style={labelStyle}>
                  Ad cost / order
                  {statsLoading && <Loader size={10} style={{ marginLeft: 4, animation: 'spin 1s linear infinite' }} />}
                </span>
              ...
              <div style={{ ...rowStyle, paddingTop: 4 }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 15, color: profitColor }}>
                  {fmt(realProfit)} MAD
                </span>
              </div>
              ...
              {stats && (
                <div style={rowStyle}>
                  ...
                  <span style={{ ...valueStyle, color: stats.return_rate > 30 ? '#f87171' : '#fbbf24' }}>
                    {stats.return_rate}%
                    <span style={{ fontSize: 10, color: '#8892b0', marginLeft: 4 }}>
                      ({stats.returned_orders} / {stats.total_orders} orders)
                    </span>
                  </span>
                </div>
              )}
        ...
        {utmRef && (
          <div>
            ...
            <div style={{ background: '#13151e', borderRadius: 10, padding: '12px 14px' }}>
              <div style={{
                fontFamily: 'monospace', fontSize: 13, color: '#00d48f',
                background: '#0d2a1e', padding: '8px 12px', borderRadius: 6,
      ...
      <div style={{ padding: '14px 20px', borderTop: '1px solid #222733', flexShrink: 0 }}>
        <button
          ...
          style={{
            width: '100%', padding: '11px 0', border: 'none', borderRadius: 8, cursor: itemId ? 'pointer' : 'not-allowed',
            background: itemId ? '#00d48f' : '#2d3248', color: itemId ? '#0d2a1e' : '#8892b0',
