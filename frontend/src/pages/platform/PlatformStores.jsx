...
import {
  ...
  Edit2, X, ChevronDown, Search, Plus, Trash2, RefreshCw, Save, Database,
} from 'lucide-react';
import {
  getPlatformStats, getPlatformStores, createPlatformStore,
...
function StorageSection({ storeId }) {
  const [data, setData]       = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getStoreStorage(storeId)
      .then(r => setData(r.data))
      .finally(() => setLoading(false));
  }, [storeId]);

  if (loading) return <div style={{ padding: '12px 0', color: 'var(--text-muted)', fontSize: 13 }}>Loading storage…</div>;
  if (!data)   return null;
  ...
  return (
    <div style={{ background: 'var(--bg)', borderRadius: 12, padding: 16, border: '1px solid var(--border)' }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
        {data.total_rows.toLocaleString()} rows · ~{data.estimated_kb} KB
      </div>
        ...
        {data.breakdown.filter(b => b.count > 0).map(b => (
          <div key={b.table}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-muted)', marginBottom: 2 }}>
              <span style={{ textTransform: 'capitalize' }}>{b.table.replace('_', ' ')}</span>
              <span>{b.count.toLocaleString()}</span>
            </div>
            <div style={{ height: 4, borderRadius: 4, background: 'var(--border)', overflow: 'hidden' }}>
              <div style={{ height: '100%', borderRadius: 4, background: 'var(--accent)', opacity: 0.7, width: `${(b.count / max) * 100}%` }} />
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{ padding: '0 20px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
            ...
            <button className="btn btn-primary" style={{ flex: 1 }} onClick={handleSubmit}
              disabled={saving || !form.username || !form.store_name || !form.password}>
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {/* ── STORAGE ── */}
          {tab === 'storage' && <StorageSection storeId={store.id} />}
...
export default function PlatformStores() {
  ...
  const handleStoreUpdate = (storeId, changes) => {
    ...
    getPlatformStats().then(r => r.data); // refresh stats silently
  };
  ...
  return (
    <div style={{ padding: '24px 20px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 22, fontWeight: 700 }}>Stores</div>
