...
with engine.connect() as conn:
    for stmt in [
        ...
        "ALTER TABLE orders ADD COLUMN reported_date TIMESTAMP",
        "ALTER TABLE leads ADD COLUMN reported_date TIMESTAMP",
        ...
        "CREATE TABLE IF NOT EXISTS warehouses (id INTEGER PRIMARY KEY, store_id INTEGER NOT NULL REFERENCES users(id), name VARCHAR NOT NULL, city VARCHAR NOT NULL, is_default BOOLEAN DEFAULT false, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
        "CREATE TABLE IF NOT EXISTS variant_stock (id INTEGER PRIMARY KEY, variant_id INTEGER NOT NULL REFERENCES variants(id), warehouse_id INTEGER NOT NULL REFERENCES warehouses(id), quantity INTEGER NOT NULL DEFAULT 0, UNIQUE(variant_id, warehouse_id))",
        "CREATE TABLE IF NOT EXISTS delivery_company_prices (id INTEGER PRIMARY KEY, store_id INTEGER NOT NULL REFERENCES users(id), company VARCHAR NOT NULL, from_city VARCHAR NOT NULL, to_city VARCHAR NOT NULL, national_fee FLOAT NOT NULL, local_fee FLOAT, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE(store_id, company, from_city, to_city))",
        ...
        "ALTER TABLE store_api_keys ADD COLUMN previous_key_expires_at TIMESTAMP",
        "CREATE TABLE IF NOT EXISTS team_member_rate_history (id INTEGER PRIMARY KEY, team_member_id INTEGER REFERENCES team_members(id), fixed_monthly FLOAT, per_order_rate FLOAT, effective_from TIMESTAMP NOT NULL, effective_to TIMESTAMP, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
        "CREATE TABLE IF NOT EXISTS bot_api_keys (id INTEGER PRIMARY KEY, store_id INTEGER NOT NULL UNIQUE REFERENCES users(id), key VARCHAR NOT NULL UNIQUE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)",
